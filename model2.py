...
class cTFA(nn.Module):
    def __init__(self, in_channels, groups, stride, r=5):
        super(cTFA, self).__init__()

        self.tra = TRA(in_channels)
        self.conv1 = nn.Conv2d(in_channels, in_channels * r, kernel_size=(3, 1), padding=(1, 0), groups=groups)
        self.prelu = nn.PReLU()
        self.conv2 = nn.Conv2d(in_channels * r, in_channels, kernel_size=(3, 1), padding=(1, 0), groups=groups)
...
class XConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, groups=1, use_deconv=False, nF=161):
        ...
        self.tra = cTFA(out_channels, groups=groups, stride=stride)
...
class XDWS_block(nn.Module):
    ...
    def __init__(self, in_channels, hidden_channels, kernel_size, stride, padding, groups, nF, use_deconv=False):
        ...
        self.tra = cTFA(hidden_channels, groups=groups, stride=stride)
...
class XMB_block(nn.Module):
    def __init__(self, in_channels, hidden_channels, kernel_size, stride, padding, groups, nF, use_deconv=False):
        ...
        self.point_conv2 = conv_module(hidden_channels, hidden_channels, 1, groups=groups)
...
if __name__ == "__main__":
    ...
    # from dataset import Dataset # Assuming dataset.py is not needed for this modification
    model = UL_UNAS()
    ...
    num_frames = magnitude.shape[0]  # 63
    input_shape = (1, num_frames, num_freq_bins)  # (1, 1, 257, 63)

    print(f"Input shape for complexity calculation: {input_shape}")
